...
const PrinterSettings: React.FC = () => {
  ...
  useEffect(() => {
    ...
    setTicketPrinter(getSavedPrinter('ticket'))
    setStickerPrinter(getSavedPrinter('sticker'))
  }, [])
  ...
  const handleSave = () => {
    try {
      ...
      if (ticketPrinter) savePrinter('ticket', ticketPrinter)
      if (stickerPrinter) savePrinter('sticker', stickerPrinter)
          ...
          <div className="col-12 col-lg-6">
            <div className="p-3 rounded border h-100">
              ...
              <div className="small text-muted">
                <div><strong>Tickets:</strong> {getSavedPrinter('ticket') || 'Sin configurar'}</div>
                <div><strong>Stickers:</strong> {getSavedPrinter('sticker') || 'Sin configurar'}</div>
              </div>
            </div>
          </div>
